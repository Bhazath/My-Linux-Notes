...
- vgcreate
- lvresize
- pvcreate : The `pvcreate` command in Linux initializes a physical volume for use by the Logical Volume Manager (LVM). This command prepares a storage device, such as a hard drive or partition, for inclusion in a volume group. Use it as follows:
